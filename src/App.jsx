...
function getDiscMain(cat,rid,ct,dr){
  const d=DISCOUNTS_MAIN.find(r=>r.cat===cat);
  if(!d)return 0;
...
export default function QuoteApp(){
  ...
  const res=useMemo(()=>{
    ...
    return(tab==="hw"?PRICE_LIST_MAIN:SVC_MAIN).filter(p=>p.pn.toLowerCase().includes(q)||p.desc.toLowerCase().includes(q)||(p.cat||"").toLowerCase().includes(q)).slice(0,22);
  },[srch,tab]);

  const addItem=useCallback(p=>{
    const disc=getDiscMain(p.cat,reg.id,ct,dr);
          ...
          <Panel title="Line Items" icon="📦" actions={
            ...
            <div style={{position:"relative",marginBottom:8}}>
              ...
              {drop&&res.length>0&&(
                <div style={{position:"absolute",zIndex:200,top:"100%",left:0,right:0,background:"white",border:`1px solid ${B}`,borderRadius:7,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",maxHeight:300,overflowY:"auto",marginTop:2}}>
                  {res.map(p=>{
                    const d=getDiscMain(p.cat||"",reg.id,ct,dr);
                    return(
            ...
            {lines.length===0?(
              ...
              <div>
                ...
                {lines.map(l=>{
                  const def=getDiscMain(l.cat,reg.id,ct,dr),ov=Math.abs(l.discount-def)>0.001;
                  const cp=l.listPrice*l.qty*(1-l.discount);
                  return(
